...
def select_data():
    ...
    try:
        ...
        sql = "SELECT education,COUNT(1) num FROM t_boss_zp_info GROUP BY education"
        cursor.execute(sql)
        # 取出每部电影的类型集合
...
def show_edunum():
    ...
    pie.set_global_opts(
        ...
        tooltip_opts=opts.TooltipOpts(is_show=True,formatter="{a} <br/>{b}: {c} ({d}%)"),
        # 标签隐藏
        legend_opts=opts.LegendOpts(is_show=True,pos_left='center',selected_mode=False)
    )
    # 生成HTML
    html="pages/iframes/edunum.html"
